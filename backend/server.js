...
// Importamos la version nativa de promesas del driver para poder usas async/await de forma limpia
...
const server = http.createServer(async (req, res) => {
    ...
    if (req.url === '/tasks' && req.method === 'GET') {
        try {
            // Ejecutamos una consulta SQL directa usando interposicion controlada del driver
            ...
            res.end(JSON.stringify({
                status: 'error en MYSQL: ' + error.message }));
        }
        return;
    }
    ...
    if (req.url === '/tasks' && req.method === 'POST') {
        ...
        // Reconstruimos el flujo de datos del cuerpo (steam data chunks)
        ...
        req.on('end', async () => {
            try {
                ...
                if (!title || !description) {
                    ...
                    res.end(JSON.stringify({ status: 'error', message: 'Faltan campos requeridos' }));
                    return;
                }
        ...
        res.end(JSON.stringify({ status: 'success', data: { task: newTask } }));
            ...
            res.writeHead(500, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ status: 'error en MYSQL: ' + error.message }));
        }
    });
    return;
    }
    ...
    if (req.url.startsWith('/task/') && req.method === 'DELETE') {
        ...
        // paso A: Cosultar a MySQL si la tarea existe y quien es el dueño
     ...
     // Paso B: si pasa el filtro, ejecutamos el borrado fisico en la tabla
        ...
        res.writeHead(500, { 'content-type': 'application/lson' });
